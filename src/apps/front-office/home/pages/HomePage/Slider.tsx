        ...
        {sliderArray.map((slide) => (
          <SwiperSlide key={slide.id}>
            <div className="relative flex items-center justify-center p-6 rounded-3xl">
              <img src={slide.image} className="object-cover rounded-3xl w-full h-[500px]"/>
              <section className="absolute top-10 left-20 flex flex-col justify-center gap-2 items-start">
                <p className="pt-[20px] text-[72px] min-w-[200px]">{slide.text1}</p>
                <p className="text-[72px] min-w-[200px]">{slide.text2}</p>
                <p className="pt-[20px] text-[30px] min-w-[200px]">{slide.text3}</p>
